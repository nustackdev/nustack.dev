import type { CodeTok } from '@/components/media/CodeSample';

/** Landing Nu code sample — persistent counter + live UI in one file. */
export const NU_SAMPLE_LINES: CodeTok[][] = [
  [{ c: 'kw', t: 'import' }, { t: ' ' }, { c: 'nu', t: 'nu' }],
  [{ c: 'kw', t: 'import' }, { t: ' ' }, { c: 'nu', t: 'nustd' }],
  [],
  [{ c: 'kw', t: 'class' }, { t: ' Counter(' }, { c: 'nu', t: 'nu.Shape' }, { t: '):' }],
  [{ t: '    value: ' }, { c: 'nu', t: 'nustd.kv.IntRef' }],
  [],
  [{ c: 'kw', t: 'class' }, { t: ' Dashboard(' }, { c: 'nu', t: 'nustd.ui.Page' }, { t: '):' }],
  [{ t: '    count: ' }, { c: 'nu', t: 'nustd.ui.TextRef' }],
  [],
  [{ c: 'kw', t: 'class' }, { t: ' App(' }, { c: 'nu', t: 'nustd.ui.Index' }, { t: '):' }],
  [{ t: '    pages = ' }, { c: 'nu', t: 'nustd.ui.Pages' }, { t: '({' }, { c: 'str', t: '"/"' }, { t: ': Dashboard})' }],
  [],
  [{ t: 'app = ' }, { c: 'nu', t: 'nu.With' }, { t: '(' }],
  [{ t: '    ' }, { c: 'nu', t: 'nustd.kv.rocksdb_navigator' }, { t: '(' }, { c: 'str', t: '".dbcounter"' }, { t: '),' }],
  [{ t: '    ' }, { c: 'nu', t: 'nustd.ui.server' }, { t: '(' }],
  [{ t: '        ' }, { c: 'nu', t: 'nustd.kv.auto_flow_atomic' }, { t: '(' }],
  [{ t: '            ' }, { c: 'nu', t: 'nu.ReactForever' }, { t: '(' }],
  [{ t: '                Counter.value.on_change(),' }],
  [{ t: '                Dashboard.count.set(Counter.value),' }],
  [{ t: '            ),' }],
  [{ t: '        ),' }],
  [{ t: '    ),' }],
  [{ t: '    body=' }, { c: 'nu', t: 'nustd.kv.auto_flow_atomic' }, { t: '(' }],
  [{ t: '        ' }, { c: 'nu', t: 'nu.IfDo' }, { t: '(Counter.value.missing(), Counter.value.set(0))' }],
  [{ t: '        >> ' }, { c: 'nu', t: 'nu.ForeverDo' }, { t: '(' }],
  [{ t: '            Counter.value.inc() >> ' }, { c: 'nu', t: 'nu.Delay' }, { t: '(1.0),' }],
  [{ t: '        )' }],
  [{ t: '    ),' }],
  [{ t: ')' }],
  [],
  [{ c: 'kw', t: 'if' }, { t: ' __name__ == ' }, { c: 'str', t: "'__main__'" }, { t: ':' }],
  [{ t: '    ' }, { c: 'kw', t: 'import' }, { t: ' asyncio' }],
  [{ t: '    asyncio.run(' }, { c: 'nu', t: 'nu.arun' }, { t: '(app))' }],
];
