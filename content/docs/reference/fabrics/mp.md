...
| Name | Sort | Call | Meaning |
| --- | --- | --- | --- |
| [MpWorkerRef](#mpworkerref) | `ref` | `MpWorkerRef(tag=<UNSET>)` | The `MpWorker` bound at `tag` on the Context. |
...
- The tag is forwarded verbatim as a single positional to `ctx.get`, so it mirrors whatever bound the worker: nothing for a bare `Provide`, the index for `ProvideList`, the key for `ProvideDict`.
