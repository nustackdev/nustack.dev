...
Nu 0.1 is out. First public release. `pip install nucore "nustd[all]" nucli` and you have the whole thing.
...
- **nustd.kv** — persistent state over RocksDB / LMDB. Transactions, snapshots, change notifications.
- **nustd.ui** — Refs as widgets. The fabric renders them in a browser and live-updates as state changes.
- **nustd.cluster** — teleport a Nu tree to any worker in your cluster; it runs there and returns. Backed by Ray.
- **nustd.llm** — one OpenAI-compatible wire, N providers (Ollama, OpenAI, OpenRouter, Groq, Cerebras, xAI, vLLM). Same ChatRef, same call.
...
- **Install & examples:** `pip install nucore "nustd[all]" nucli` — [examples on GitHub](https://github.com/nustackdev/nu/tree/main/examples)
- **Demo:** `nu demo movies`
